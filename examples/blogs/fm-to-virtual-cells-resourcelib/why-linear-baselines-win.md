---
title: Why do single-cell foundation models fail to beat linear baselines?
date: '2026-04-03'
topics:
- eval
...
**The track that addresses this**: Track 2 in the talk's small-lab supplementary — counterfactual pretraining objectives (IRM, contrastive perturbation losses, energy-based causal priors).
...
**The track that addresses this**: Track 4 in the talk's small-lab supplementary — graph-attention, pathway priors, lineage-aware encoders.
...
**The track that addresses this**: Track 6 in the talk's small-lab supplementary — causal-recovery benchmarks with MR-validated + LINCS L1000 + ENCODE TF-target test sets.
...
**The track that addresses this**: [interpretability, mech-modeling, causal](interpretability-mech-causal.md) — the cancer-curated SAE angle is the small-lab differentiator now that the general-domain wave has broken.
...
**The paper demonstrates** the framework on a 22M-cell immunology SOTA model. It **names** the problem but does **not** give the field a transportability benchmark — that's [the causal-transportability frontier](interpretability-mech-causal.md), the wide-open frontier project.
...
*Interactive — the bridge from diagnosis to project: rows are the 9 small-lab innovation tracks from the talk's supplementary, columns are the four causes plus the theoretical framing. Every cause has at least one track aimed directly at it. Hover a cell for the track's open problem.*

## Where to go next

- **[Foundation models — state of play](foundation-models-state-of-play.md)** — the chronological narrative of the reckoning sits inside the cross-family map.
- **[The evaluation papers catalog](evaluation-papers-catalog.md)** — all 10 reckoning papers + the contrarian + the substrate papers.
- **[Small labs — what to build](small-labs-what-to-build.md)** — the small-lab projects that address each cause above.
