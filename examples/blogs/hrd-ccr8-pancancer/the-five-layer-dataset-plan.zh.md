...
Phase-B 调研里浮出三条 YAML 需要修正，应在后续提交里改（不在本博客范围内）：

1. **Launonen 2022 DOI**：语料 YAML 里写的 `10.1038/s41467-022-32310-3` 解析不出来。正确的 DOI 是 `10.1038/s41467-022-28389-3`（*Nat Commun* 2022，13:835）。
2. **Hwang 2022 空间模态**：语料 YAML 的 `why` 文本把 Hwang 2022 写成 PDAC + Slide-seq。发表版（Hwang et al. 2022 *Nat Genet*）用的是 NanoString GeoMx Whole-Transcriptome DSP，做在 21 个肿瘤上，不是 Slide-seqV2。Slide-seq 可能对应 Hwang 实验室的另一篇姊妹存档，但不属于这份语料的锚定文献。
3. **Stur 2022 分层**：语料 YAML 暗示 Visium 队列按 BRCA 状态分层。发表版 Stur 2022 *iScience* **没有**报告 BRCA 状态；队列分组用的是 RECIST 1.1 NACT 应答（6 例 excellent vs 5 例 poor 应答者）。BRCA 分层的那篇 Stur 是另一篇 2023 年的 preprint，不是这篇 iScience。语料里的 rationale 要改 —— Stur 2022 支撑的是 NACT-应答这一层分析，不是 BRCA 分层的空间分析。

这些不阻塞本计划 —— 上面的计划本就是把 Stur 2022 当作 NACT 应答分层来用的 —— 但应反映到 YAML 里，让每项的 `why` 与 `doi` 字段是准确的。
