...
| **Magen 2023 HCC [v]** | standalone | HCC (anti-PD-1) | WES | scRNA + scTCR | 33 | **accession 没查到**（待核） |
| **Wang 2025 SCLC PARPi [v]** | standalone | SCLC (PARPi context) | panel | scRNA | verify | **accession 未核实** |
...
| **Erickson 2022 前列腺 Visium** | standalone | 前列腺腺癌 | WGS + low-pass WGS | Visium（同病人多切片） | 11 | EGA `EGAS00001006124` |
| **Khaliq / Sun 2024 PDAC 原发 + 转移** | standalone | PDAC（原发 + 肝/LN 转移） | WES + bulk RNA | Visium | 21 | GEO `GSE272362` |
| **Pei / Min 2025 PDAC rapid autopsy** | standalone | PDAC（尸检多脏器） | WGS/WES（尸检） | Visium（55 切片） | 13 | **accession 未核实**（Nature 642:212–221） |
| **Wu 2025 HGSOC Visium HD（preprint）** | standalone | HGSOC | bulk WES + HRD | Visium HD | ~30 | **accession 未核实**（preprint `10.1101/2025.11.24.690313`） |
...
| **Risom 2022 DCIS MIBI (HTAN-Stanford)** | HTAN-HTA6-orbital | breast DCIS → IBC | targeted panel + bulk RNA | MIBI-TOF, 37-plex | 122 | Synapse via HTAN-HMS（`syn17773547` 候选 —— 待核） |
...
| **Magness / Enfield 2024 TRACERx 100 IMC** | TRACERx (CRUK / UCL) | NSCLC 早期 | 多区域 WES + bulk RNA | IMC（2 panel） | 81 | Zenodo `12587543`（controlled-access via request） |
...
| **Owkin MOSAIC-Window（膀胱子集）** | **MOSAIC** (Owkin + 5 EU 中心) | 膀胱 (MIBC) —— MOSAIC 首次公开释放 | WES + bulk RNA | Visium + Chromium Flex snRNA + H&E | **15** 公开（MIBC 子集 / 全队列 ~2,500 已采） | EGA `EGAD50000001251` (parent `EGAS50000000689`) |
...
| **Hwang 2025 PDAC 神经侵犯** | standalone | PDAC | bulk panel（verify） | Visium + snRNA | 25（62 samples） | **accession 未核实** |
| **Sun 2024 HCC 原发 + 转移** | standalone (Fudan) | HCC | 多区域 WES + bulk RNA | Visium + scRNA + IHC | **182（257 原发 + 176 转移）** | **accession 没查到**（待核） |
...
| **Greenwald 2024 GBM (Suvà/Tirosh)** | standalone | GBM + IDH-mut glioma | bulk WES（子集 + TCGA-anchored） | Visium + MIBI + scRNA + scATAC + Patch-seq | 100（>1M cells） | GEO `GSE237183` + Zenodo `10.5281/zenodo.8105466` |
...
发表后做了一轮 accession 验证，抓出了几个错的 ID，也理清了访问模式。按 repo 大致这样：

- **Open**（不用申请，谁都能下）：所有 GEO `GSE…`、ArrayExpress `E-MTAB-…`、SRA `SRP…`、Broad Single Cell Portal `SCP…`、Zenodo、PRIDE。A 桶绝大多数 + B/C 桶里的 standalone 都在这一档。
- **Controlled-access**（要 DAC 申请，一般 2–6 周）：dbGaP `phs…`、EGA `EGAS…` / `EGAD…`。metadata 公开；raw FASTQ/BAM 是 gated。绝大多数 IO 队列（BIOKEY、CM-038、ADAPTeR 等）、HTAN 的 raw 基因组层、Owkin MOSAIC 都在这一档。
- **Synapse —— 混合**：processed L3/L4 + metadata 用免费 Synapse 账号就能下。raw L1/L2 通过配套的 dbGaP/EGA tier 控制。HTAN master fileview `syn20446927` 是 gateway —— 浏览随便，但每个子 atlas 的原始测序数据要走对应 DUA。HMS-LSP 的几个项目（Färkkilä、Launonen、HMS-Sorger 卵巢）放的是 CyCIF/mIF 处理后的图像 + segmentation 表，开放。
- **GSA（中国 NGDC）** —— `HRA…` ID 通常是 controlled-access（HDAC 申请）。
- **Owkin MOSAIC-Window**：`EGAD50000001251` 这次公开的 specifically 是 MOSAIC 整体 ~2,500 例池子里的 **15 例 muscle-invasive 膀胱子集**。规划按 15 例，不是 60。

原稿里有 6 个 accession ID 指错了队列，上面表里都已纠正：

- Erickson 2022 前列腺 Visium：`EGAD00001008622`（其实是 EPICC CRC）→ 改成 `EGAS00001006124`。
- Magness/Enfield 2024 TRACERx 100 IMC：`EGAS00001000809`（其实是 TRACERx pilot）→ 改成 Zenodo `12587543`。
- Greenwald 2024 GBM："GEO + Synapse" → 改成 `GSE237183` + Zenodo `10.5281/zenodo.8105466`。
- Mitri 2024 AMTEC："dbGaP per paper" → 确认是 `phs002371.v1.p1`（HTAN-HTAPP 总号）。
- Sun 2024 HCC：`HRA005020` 其实是四川大学一个胰岛分化的研究 → 标 `accession 没查到`。
- Magen 2023 HCC：`phs002748` 其实是 Rosenberg metastectomy TCR 那个 → 标 `accession 没查到`。
- Risom 2022 DCIS MIBI：`syn26844071` 其实是 Lee 2020 CRC 项目 → 标 `syn17773547` 为候选，依赖前先核。

四个 2025 年的队列（Hwang Cancer Cell PDAC 神经侵犯、Pei Nature PDAC autopsy、Wu HGSOC Visium HD preprint、Wang SCLC PARPi）太新，accession 没出现在公开索引里 —— 它们那一行都标了 `accession 未核实`。等发表了再补。

教训：agent 给的 accession ID 必须直接去查源，不能只按 repo policy 分类。四桶清单 + 病人数本身是稳的；错的是 6 个 ID 的转录 / 匹配 mismatch，不是队列本身。
